...
sub test_stringForFile {
    ...
    my $text =
      $stringifier->stringForFile(
        $this->{attachmentDir} . 'Simple_example.pptx' );
    ...
    $this->assert_str_equals( $text, $text2,
        "PPTX stringifier not well registered." );
...
sub test_passwordedFile {
    ...
    my $text =
      $stringifier->stringForFile(
        $this->{attachmentDir} . 'Passworded_example.pptx' );
...
sub test_maliciousFile {
    ...
    my $text =
      $stringifier->stringForFile( $this->{attachmentDir} . 'Im_a_png.pptx' );
