...
sub test_stringForFile {
    ...
    my $text =
      $stringifier->stringForFile(
        $this->{attachmentDir} . 'Simple_example.ppt' );
    my $text2 = Foswiki::Contrib::Stringifier->stringFor(
        $this->{attachmentDir} . 'Simple_example.ppt' );

    $this->assert( defined($text) && $text ne "", "No text returned." );
    $this->assert_str_equals( $text, $text2,
        "PPT stringifier not well registered." );
...
sub test_passwordedFile {
    ...
    my $text =
      $stringifier->stringForFile(
        $this->{attachmentDir} . 'Passworded_example.ppt' );

    # not too sure what to test... This is the default from pptHtml
    $this->assert_matches( 'Created with pptHtml', $text )
      ;    #, "Protected file generated some text?");
}
...
sub test_maliciousFile {
    ...
    my $text =
      $stringifier->stringForFile( $this->{attachmentDir} . 'Im_a_png.ppt' );
