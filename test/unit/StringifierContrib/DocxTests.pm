...
sub test_stringForFile {
    ...
    $this->assert_str_equals( $text, $text2,
        "DOCX stringifier not well registered." );
...
sub test_SpecialCharacters {
    ...
    my $text =
      $stringifier->stringForFile(
        $this->{attachmentDir} . 'Simple_example.docx' );
...
sub test_passwordedFile {
    ...
    my $text =
      $stringifier->stringForFile(
        $this->{attachmentDir} . 'Passworded_example.docx' );
...
sub test_maliciousFile {
    ...
    my $text =
      $stringifier->stringForFile( $this->{attachmentDir} . 'Im_a_png.docx' );
