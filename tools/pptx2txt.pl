...
my $notes;
while (
/<Override PartName="(\/ppt\/notesSlides\/.*?)" ContentType=".*?\/([^\/]*?notesSlide\+xml)"?\/>/g
  )
{
    $notes{"$1"} = $2;
}
...
$content =~
s{<w:pPr><w:jc w:val="center"/></w:pPr><w:r><w:t>(.*?)</w:t></w:r>}/cjustify($1)/oge;

$content =~
s{<w:pPr><w:jc w:val="right"/></w:pPr><w:r><w:t>(.*?)</w:t></w:r>}/rjustify($1)/oge;
